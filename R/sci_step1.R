...
usage_str <- paste0(paste0("\033[93m<workdir>", RESET), ' ', paste0("\033[93m<scratch>", RESET), ' ', paste0("\033[92m<sample_name>", RESET), ' ', paste0("\033[92m<umi_cutoff>", RESET))

args_raw <- commandArgs(trailingOnly = TRUE)

if (length(args_raw) != 4) {
  cat(WHITE, 'Usage: Rscript sci_step1.R ', usage_str, RESET, '\n\n', sep = '')
  cat_col("Used in combination with SCI_2, from fastq files produces gene expression matrix and QC plots and statistics for SCI data", color = YELLOW)
  cat('\n')
  ...
  cat('\033[92msample_name           a character string indicating the name of the experiment', RESET, '\n', sep = '')
  cat('\033[92mumi_cutoff            minimum number of UMI per cell to consider the cell valid', RESET, '\n', sep = '')
  quit(status = 1)
}
...
args$sample_name <- args_raw[3]
args$umi_cutoff <- args_raw[4]
...
mounts <- c(mounts, paste0('-v "', normalizePath(args$scratch), ':/data/scratch"'))
docker_vals$scratch <- '/data/scratch'

# --- Bind files and service volumes ---
mounted_folders <- list()

docker_vals$sample_name <- args$sample_name
docker_vals$umi_cutoff <- args$umi_cutoff

# --- Assemble docker command ---
mount_str <- paste(mounts, collapse = ' ')
cmd <- paste('docker run --rm', mount_str, 'repbioinfo/sci_tomatrix_genome /home/tomatrix.sh <sample_name> <umi_cutoff>')
