...
usage_str <- paste0(paste0("\033[93m<workdir>", RESET), ' ', paste0("\033[93m<scratch>", RESET))

args_raw <- commandArgs(trailingOnly = TRUE)

if (length(args_raw) != 2) {
  cat(WHITE, 'Usage: Rscript sci_step2.R ', usage_str, RESET, '\n\n', sep = '')
  cat_col("Used in combination with SCI_1, from fastq files produces gene expression matrix and QC plots and statistics for SCI data", color = YELLOW)
  cat('\n')
  cat_col('Arguments:', color = WHITE)
  cat('\033[93mworkdir         [io]  Path to working directory containing scratch folder', RESET, '\n', sep = '')
  cat('\033[93mscratch         [io]  path of the directory output of the sci_step1 function', RESET, '\n', sep = '')
  quit(status = 1)
}
...
cmd <- paste('docker run --rm', mount_str, 'repbioinfo/monocle Rscript /home/endpipeline.R ')
