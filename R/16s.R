...
if (length(args_raw) != 2) {
  cat(WHITE, 'Usage: Rscript 16s.R ', usage_str, RESET, '\n\n', sep = '')
  cat_col("Pipeline for the analysis of 16S rRNA gene sequencing data", color = YELLOW)
  cat('\n')
...
print(mount_str)
cmd <- paste('docker run --rm', mount_str, 'repbioinfo/qiime2023 /home/qiime_full.sh ')
