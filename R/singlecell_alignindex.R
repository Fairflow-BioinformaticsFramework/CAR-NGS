...
if (length(args_raw) != 4) {
  cat(WHITE, 'Usage: Rscript singlecell_alignindex.R ', usage_str, RESET, '\n\n', sep = '')
  cat_col("Index and Alignment Script for single cell analysis", color = YELLOW)
  cat('\n')
  ...
  cat('\033[92mbamsave               variable indicating if the BAM files are to be saved', RESET, '\n', sep = '')
  quit(status = 1)
}
...
if (!args$bamsave %in% c("true", "false")) {
  errors <- c(errors, paste0('Invalid value for bamsave: ', args$bamsave, '. Allowed: true, false'))
}
...
cmd <- paste('docker run --rm', mount_str, 'repbioinfo/carncellranger2 /home/index_align.sh <bamsave>')
