...
if (length(args_raw) != 3) {
  cat(WHITE, 'Usage: Rscript heatmap.R ', usage_str, RESET, '\n\n', sep = '')
  cat_col("Generates heatmaps based on a gene count matrix", color = YELLOW)
  cat('\n')
...
cmd <- paste('docker run --rm', mount_str, 'repbioinfo/rnaseqstar_v2 Rscript /home/heatmap.R <matrix_file> <metadata_file>')
